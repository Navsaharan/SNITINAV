...
// GET /api/imap/config - IMAP client configuration
export const GET = buildSafeApiRoute(async (request: NextRequest) => {
  ...
  switch (format.toLowerCase()) {
    ...
    default:
      return NextResponse.json({
        service: 'SNPITC IMAP Configuration',
        ...config,
        instructions: {
          ...
          folders: 'Standard folders: INBOX, Sent, Drafts, Trash, Spam'
        },
...
function generateOutlookConfig(config: any) {
  return {
    ...
    steps: [
      ...
      '5. Click "More Settings" for advanced options',
...
function generateThunderbirdConfig(config: any) {
  return {
    ...
    steps: [
      ...
      '4. Enter the server settings above',
      '5. Configure folder settings in Advanced tab',
      '6. Click "Re-test" and then "Create Account"'
    ]
  }
}
...
function generateAppleMailConfig(config: any) {
  return {
    ...
    steps: [
      ...
      '5. Configure mailbox behaviors in Advanced settings',
      '6. Verify and save the account'
    ]
  }
}
...
function generateAndroidConfig(config: any) {
  return {
    ...
    steps: [
      ...
      '4. Enter the server settings above',
      '5. Configure folder settings if available',
